...
#### 2. 이모지 반응 (⭐ 가장 간단) - ✅ UI 완료!
- [x] 메시지 우클릭으로 반응 선택 가능
- [x] 메시지 호버 시 빠른 반응 버튼 표시
- [x] 👍 ❤️ 😂 🎉 😮 😢 기본 이모지 지원
- [x] 반응 클릭해서 토글 가능
- [x] 반응 개수 표시 및 본인 반응 강조
- 난이도: ⭐⭐ (중간)
- 예상 시간: 2-3시간
- **상태**: ✅ UI 구현 완료 (Commit: `48947e6`)
- **구현된 기능**:
  - Right-click context menu (6개 이모지 피커)
  - Hover quick reactions (상위 3개 빠른 버튼)
  - 반응 토글 추가/삭제
  - 반응 카운트 표시
  - 현재 사용자 반응 시각적 강조
  - 화면 경계 감지해서 menu 위치 자동 조정
...
- [x] 시간 제한 (5분 이내)
- [x] "수정됨" 표시
- [x] `is_edited`, `edited_at` 컬럼 활용
- 난이도: ⭐⭐ (중간)
- 예상 시간: 2-3시간
- **상태**: ✅ 완료 (Commit: `50d9fdf`)
- **구현된 기능**:
  - 본인 메시지에만 Edit 버튼(✏️) 표시
  - 메시지 발송 후 5분 이내에만 편집 가능
  - Textarea로 메시지 수정
  - Ctrl+Enter로 저장, Escape로 취소
  - 수정된 메시지에 "(수정됨)" 라벨 표시
  - Supabase에 is_edited=true, edited_at 저장
  - 5분 이후 자동으로 Edit 버튼 사라짐

#### 6. 메시지 답장 (Reply) - ✅ 완료!
- [x] 특정 메시지에 답장 기능
- [x] 원본 메시지 미리보기 표시
- [x] `parent_message_id` FK 추가
- [x] 답장 연결 시각적 표현 (인덴트/라인)
- 난이도: ⭐⭐⭐ (중상)
- 예상 시간: 3-4시간
- **상태**: ✅ 완료 (Commit: `5060b4b`)
- **구현된 기능**:
  - 메시지 호버 시 답장 버튼(↩️) 표시
  - 클릭하면 replied message 상태 활성화
  - ChatInput에 반응 미리보기 표시 (X로 취소 가능)
  - 회신 메시지에 부모 메시지 미리보기 표시 (↪️ 표시)
  - 부모 메시지 내용 1줄로 표시 (truncate)
  - 왼쪽 보더로 시각적 구분
  - 메시지 발송 후 자동으로 회신 모드 종료

#### 7. 이모지 반응 고도화 (DB 저장 + Realtime)
...
🌐 **배포 상태:**
- GitHub: ✅ 최신 커밋 푸시됨 (`5060b4b`)
- Vercel: ✅ 자동 배포 완료 (배포 대기 중)
- 배포된 앱: 4가지 기능 정상 작동 ✅

**📌 작업 진행도:**
- Phase 1: 100% ✅ (6개 기능 완료)
- Phase 2: 80% (4/5 기능 완료)
  - ✅ 메시지 읽음 표시 개선
  - ✅ 이모지 반응 (UI)
  - ✅ 메시지 편집 (5분 제한)
  - ✅ 메시지 답장 (부모 메시지 미리보기)
  - 📋 이모지 반응 고도화 (DB + Realtime)
