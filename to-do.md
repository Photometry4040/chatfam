...
#### 6. 음성 메시지
- [ ] 버튼 길게 눌러 녹음 - 취소
- [ ] MediaRecorder API 사용 - 취소
- [ ] Supabase Storage에 저장 - 취소
- 난이도: ⭐⭐⭐ (중상)
...
- [x] 특정 메시지에 답장
- [x] 원본 메시지 미리보기
- [x] `parent_message_id` FK 추가
- 난이도: ⭐⭐⭐ (중상)

#### 9. 이모지 반응 고도화 (DB 저장 + Realtime) - ✅ 완료!
- [x] `chat_message_reactions` 테이블 추가
- [x] 반응 Supabase에 저장
- [x] Realtime 구독으로 실시간 업데이트
- [x] 반응한 사람 목록 표시 (hover tooltip)
- 난이도: ⭐⭐⭐ (중상)
- **상태**: ✅ 완료 (Commit: `0af5a62`)
- **구현된 기능**:
  - Composite key (message_id, user_id, emoji) 사용
  - RLS 정책으로 보안 강화
  - message_id, user_id, emoji 인덱스 추가
  - 초기 로드 시 DB에서 반응 데이터 가져오기
  - Real-time 구독으로 반응 변경 감지
  - 낙관적 업데이트로 즉각적인 UI 반응
  - 반응한 사용자 이름을 hover tooltip으로 표시
  - 반응 추가/제거 시 데이터베이스에 즉시 저장
...
- 메시지 읽음 표시 개선 - ✅ **완료** (IntersectionObserver 기반)
  ...
  - Commit: `b6046d1`
- 이모지 반응 UI - ✅ **완료**
  - Right-click context menu로 이모지 선택
  - Hover 시 빠른 반응 버튼 표시
  - 6개 기본 이모지 지원
  - Commit: `48947e6`
- 메시지 편집 - ✅ **완료** (5분 제한)
  - 본인 메시지만 수정 가능
  - 메시지 발송 후 5분 이내만 편집 가능
  - "(수정됨)" 라벨 표시
  - Commit: `50d9fdf`
- 메시지 답장 - ✅ **완료**
  - 메시지에 답장 기능
